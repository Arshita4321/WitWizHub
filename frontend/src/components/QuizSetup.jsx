...
const QuizSetup = ({ onQuizGenerated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    onQuizGenerated(subject, topicsArray, parsedNum, level);
  };

  const handleNumQuestionsChange = (e) => {
    const val = e.target.value;
    // Allow empty string while typing; store as number only if valid
    if (val === '') {
      setNumQuestions('');
      return;
    }
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="text-center mb-8">
        ...
        <motion.p variants={itemVariants} className="text-xl text-gray-300 font-light">
          Create your personalized quiz experience
        </motion.p>
      </motion.div>

      {/* Main Form Container */}
      <motion.form onSubmit={handleSubmit} variants={itemVariants} className="relative">
        {/* Glassmorphism Card */}
